 ...
 * `.none`, then up to 5 faces will be returned with detected contours.
 *
 * <p>`performanceMode` set to `.accurate`, or if `classificationMode` or `landmarkMode` is set to
 * `.all`, then all detected faces will be returned, but only up to 5 faces will have detecteted
 * contours.
 */
...
/**
 * Whether the face tracking feature is enabled for face detection. For tracking a face in a video
 * feed, setting this to YES will improve latency. Defaults to NO.
 */
